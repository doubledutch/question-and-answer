...
export default class MyList extends Component {
  constructor(props) {
    super(props)
    this.checkmarkSelectedBackground = new Color(props.primaryColor)
      .limitSaturation(0.5)
      .minLightness(0.9)
      .limitLightness(0.9)
      .rgbString()
    ...
    <View>
      <View style={s.listContainer}>
        ...
        <View style={s.rightContainer}>
          ...
          {item.anom === true && <Text style={s.nameText}>-Anonymous</Text>}
        </View>
      </View>
  ...
  renderHelpText = () => {
    ...
    return (
      <View style={{ marginTop: 96 }} b>
        ...
        >
          {moderator[0].approve === false && currentSort === 'New'
            ? t('mod_off')
        ...
        <View style={{ height: 60 }}>
          <View style={s.buttonContainer}>
            <TouchableOpacity style={s.squareHeaderButton1} onPress={this.props.showAdminPanel}>
              <Text style={[s.adminDashboardButton, this.primaryColorStyle()]}>
                {this.props.openAdminHeader ? t('hide') : t('open')} Admin Panel
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.squareHeaderButton2} onPress={this.props.renderFilterSelect}>
              <Text style={s.adminDashboardButtonTitle}>Filters: </Text>
              <Text style={[s.adminDashboardButton, this.primaryColorStyle()]}>
                {' '}
      ...
      case 'New':
        return (
          <View style={[{ height: 44, backgroundColor: 'white' }, s.buttonContainer]}>
            <TouchableOpacity
              style={s.squareHeaderButton1}
              onPress={() => this.props.changeQuestionStatus(question, 'block')}
            >
              <Image style={{ height: 20, width: 20 }} source={deletecircle} />
              <Text style={s.adminDashboardButtonRed}> {t('block')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.squareHeaderButton2}
              onPress={() => this.props.changeQuestionStatus(question, 'approve')}
            >
              <Image style={{ height: 20, width: 20 }} source={checkcircle} />
              <Text style={s.adminDashboardButtonGreen}> {t('approve')}</Text>
            </TouchableOpacity>
          </View>
        )
      case 'Answered':
      ...
      case 'Approved':
        return (
          <View style={[{ height: 44, backgroundColor: 'white' }, s.buttonContainer]}>
            ...
            <TouchableOpacity
              style={s.squareHeaderButton2}
              onPress={() => this.props.changeQuestionStatus(question, 'answer')}
            >
              <Image style={{ height: 20, width: 20 }} source={checkmark} />
              <Text style={s.adminDashboardButtonBlue}> {t('answer')}</Text>
            </TouchableOpacity>
          </View>
        )
    }
  ...
  renderVoteButton = question => (
    <TouchableOpacity style={s.leftContainer} onPress={() => this.newVotes(question)}>
      <Image
        style={[s.checkmark, question.myVote ? this.checkmarkSelectedStyle() : null]}
        source={thumbsup}
      />
