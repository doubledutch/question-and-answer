...
export default class PresentationDriver extends PureComponent {
  publicSessionRef = props =>
    (props || this.props).fbc.database.public
      .adminRef('sessions')
      .child((props || this.props).session)
    ...
    if (!session || !publicSession)
      return (
        <div className="presentation-driver">
          <button onClick={this.initializeSession}>{t('initialize')}</button>
        </div>
      )

    switch (publicSession.state) {
      case 'LIVE':
        return <div className="presentation-driver">{this.renderClose()}</div>
      default:
        return <div className="presentation-driver">{this.renderStart()}</div>
    }
  }

  renderStart = () => (
    <button className="tertiary" onClick={this.startSession}>
      {t('session_start')}
    </button>
  )

  renderClose = () => (
    <button className="tertiary" onClick={this.endSession}>
      {t('session_close')}
    </button>
  )
