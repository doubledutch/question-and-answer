...
export class CellEdit extends Component {
  ...
  renderIcons = task => {
    if (this.state.action === 'state') {
      return (
        <div className="rightButtons">
          <button className="borderlessButtonSmall" onClick={this.handleEdit} value="false">
            {t('edit_verb')}
          </button>
          <button className="borderlessButton" onClick={this.handleDelete} value="false">
            {task.archive ? t('display_in_app') : t('hide_in_app')}
          </button>
        </div>
      )
    }
    ...
    if (this.state.action === 'delete') {
      return (
        <div className="rightButtons">
          <button className="borderlessButton" value="cancel" onClick={this.handleBlur}>
            {t('cancel')}
          </button>
          <button className="borderlessButton" value="delete" onClick={this.confirmDelete}>
            {t('confirm')}
          </button>
        </div>
      )
    ...
    if (this.state.action === 'edit') {
      return (
        <div className="rightButtons">
          <button className="borderlessButton" value="edit" onClick={this.confirmEdit}>
            {t('done')}
          </button>
        </div>
      )
  ...
  confirmEdit = () => {
    const named = this.state.value.trim()
    let status = true
    if (named) {
      for (const item of this.props.sessions) {
