...
export class SessionBox extends Component {
  ...
  render() {
    const sessions = this.createList()
    return (
      <div>
        <div className="cellBoxTop">
          <h2>{t('sessions')}</h2>
          {this.props.hideSessions ? null : (
            <button className="addSessionButton" onClick={this.handleNewSession} value="save">
        ...
        {this.props.hideSessions ? null : (
          <div>
            <label className="boxTitleBold" style={{ marginLeft: 10 }}>
              {t('name')}
            </label>
            <div className="sessionListBox">
              ...
              {sessions.length === 0 ? (
                <p className="sessionBoxHelpText">{t('no_sessions')}</p>
              ) : null}
            </div>
          </div>
        )}
    ...
    <div>
      <span className="textInputBox">
        ...
        <div className="rightButtons">
          <button className="borderlessButtonMed" onClick={this.handleSubmit} value="save">
            {t('save')}
          </button>
          <button className="borderlessButton" onClick={this.handleClose} value="false">
            {t('cancel')}
          </button>
        </div>
      </span>
  ...
  handleSubmit = (event, keyPress) => {
    let status = true
    const sessionName = this.state.value.trim()
    if (sessionName) {
